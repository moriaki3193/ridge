...
PATH2RATINGS = path.join(path.dirname(path.abspath(__file__)), 'tmp', 'ratings.npy')
...
class TestMatFac(unittest.TestCase):
    ...
    def test_matrix_factorization(self):
        # [START 入力特徴量の作成]
        ratings = None
        if path.exists(PATH2RATINGS):
            ratings = np.load(PATH2RATINGS)
        else:
